...
import { GalleryEntry } from "./GalleryEntry.tsx";
...
export function ShikeApp() {
  ...
  const [homeTab,setHomeTab]=useState<"create"|"gallery">("create");
  ...
  const t = makeT(lang);
  return <I18nContext.Provider value={{ lang, t }}><main className="shike"><div className="shell"><header className="topbar"><button className="brand" onClick={()=>setStage(0)} style={{border:0,background:"none",padding:0,cursor:"pointer"}}>拾刻 <small>SHIKE</small></button>{stage===0&&<div className="home-tabs" role="tablist" aria-label={t("home_tabs_label")}><button role="tab" aria-selected={homeTab==="create"} className={`home-tab ${homeTab==="create"?"active":""}`} onClick={()=>setHomeTab("create")}>{t("tab_create")}</button><button role="tab" aria-selected={homeTab==="gallery"} className={`home-tab ${homeTab==="gallery"?"active":""}`} onClick={()=>setHomeTab("gallery")}>{t("tab_gallery")}</button></div>}<div className="lang-switch"><button className={`lang-btn ${lang==="zh"?"active":""}`} onClick={()=>setLang("zh")}>中</button><span className="lang-sep">/</span><button className={`lang-btn ${lang==="en"?"active":""}`} onClick={()=>setLang("en")}>EN</button></div></header>
    <AnimatePresence mode="wait">
      {stage===0&&<motion.section key="home" className="hero" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><WorldMapCanvas/>{homeTab==="create"&&<div className="hero-copy"><div className="eyebrow">{t("hero_eyebrow")}</div><h1 dangerouslySetInnerHTML={{__html:t("hero_title")}}/><p>{t("hero_desc")}</p><nav className="entry-grid" aria-label={t("entry_a_strong")}><button className="entry-card entry-primary" onClick={()=>setStage(7)}><span>{t("entry_a_span")}</span><strong>{t("entry_a_strong")} <b aria-hidden="true">→</b></strong><small>{t("entry_a_small")}</small></button><button className="entry-card entry-secondary" onClick={()=>setStage(6)}><span>{t("entry_b_span")}</span><strong>{t("entry_b_strong")} <b aria-hidden="true">→</b></strong><small>{t("entry_b_small")}</small></button></nav></div>}{homeTab==="gallery"&&<div className="gallery-view"><GalleryEntry onBack={()=>setHomeTab("create")}/></div>}</motion.section>}
